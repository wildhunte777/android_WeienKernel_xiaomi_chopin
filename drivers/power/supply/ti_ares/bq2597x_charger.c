...
static int bq2597x_set_charge_mode(struct bq2597x *bq, int mode)
{
	int ret = 0;
	u8 val = 0;
	...
	if (mode == SC8551_CHARGE_MODE_BYPASS && bq->bypass_mode_enable != 1)
		return -EPERM;
...
static int bq2597x_disable_vbus_inrge_det(struct bq2597x *bq, bool disable)
{
	int ret = 0;
	u8 val;
	...
	if (disable)
		val = SC8551_VBUS_INRGE_DET_DISABLE;
	else
		val = SC8551_VBUS_INRGE_DET_ENABLE;

	val <<= SC8551_VBUS_INRGE_DET_DIS_SHITF;

	ret = bq2597x_update_bits(bq, SC8551_REG_35,
				SC8551_VBUS_INRGE_DET_DIS_MASK, val);
	return ret;
}
...
static int bq2597x_parse_dt(struct bq2597x *bq, struct device *dev)
{
	...
	if (bq->chip_vendor == SC8551) {
		...
		bq->cfg->sc8551a_dual_standalone = of_property_read_bool(np,
...
static int sc8551_set_bypass_mode_en(struct charger_device *chg_dev, int enable)
{
	struct bq2597x *bq = charger_get_data(chg_dev);
...
static int sc8551_get_bypass_mode_en(struct charger_device *chg_dev, int *bypass_en)
{
	struct bq2597x *bq = charger_get_data(chg_dev);
	...
	*bypass_en = bq->bypass_mode_enable;
	return 0;
}

static int sc8551_set_charge_mode(struct charger_device *chg_dev, int mode)
{
	struct bq2597x *bq = charger_get_data(chg_dev);
	int ret = 0;
	u8 val = 0;

	ret = bq2597x_set_charge_mode(bq, mode);

	/* in bypass mode, ovp will be set to half value automatically */
	/* in charge_pump mode, should set it manually */
	if (!ret && mode == SC8551_CHARGE_MODE_DIV2) {
		ret = bq2597x_set_acovp_th(bq, bq->cfg->ac_ovp_th);
		ret = bq2597x_set_busovp_th(bq, bq->cfg->bus_ovp_th);
	}
...
static int sc8551_get_charge_mode(struct charger_device *chg_dev, int *mode)
{
	struct bq2597x *bq = charger_get_data(chg_dev);
	int ret = 0;
	u8 val = 0;
	...
	*mode = (int)(val & SC8551_CHARGE_MODE_MASK);

	return ret;
}
EXPORT_SYMBOL_GPL(sc8551_get_charge_mode);

static int bq2597x_get_battery_present(struct charger_device *chg_dev, int *batt_pres)
{
	struct bq2597x *bq = charger_get_data(chg_dev);
	int ret;
	u8 reg_val;
...
static int bq2597x_get_vbus_present(struct charger_device *chg_dev, int *vbus_pres)
{
	struct bq2597x *bq = charger_get_data(chg_dev);
	int ret;
	u8 reg_val;
...
static int bq2597x_get_alarm_status(struct charger_device *chg_dev, int *alarm_status)
{
	struct bq2597x *bq = charger_get_data(chg_dev);
...
static int bq2597x_get_fault_status(struct charger_device *chg_dev, int *fault_status)
{
	struct bq2597x *bq = charger_get_data(chg_dev);

	bq2597x_check_fault_status(bq);

	*fault_status = ((bq->bat_ovp_fault << BAT_OVP_FAULT_SHIFT)
...
static int bq2597x_get_vbus_error_status(struct charger_device *chg_dev, int *vbus_error_status)
{
	struct bq2597x *bq = charger_get_data(chg_dev);
	int ret;
	u8 stat = 0;
	...
	if (!ret) {
		if (stat & VBUS_ERROR_LO)
			*vbus_error_status = VBUS_ERROR_LOW;
...
static int bq2597x_get_reg_status(struct charger_device *chg_dev, int *reg_status)
{
	struct bq2597x *bq = charger_get_data(chg_dev);
	int ret;

	ret = bq2597x_check_reg_status(bq);
	*reg_status = (bq->vbat_reg << VBAT_REG_STATUS_SHIFT) |
...
static int bq2597x_enable_charge(struct charger_device *chg_dev, bool enable)
{
	struct bq2597x *bq = charger_get_data(chg_dev);
	int ret;
	u8 val;
...
static int bq2597x_check_charge_enabled(struct charger_device *chg_dev, bool *enabled)
{
	struct bq2597x *bq = charger_get_data(chg_dev);
	int ret;
	u8 val;
...
static int bq2597x_set_bus_protection(struct charger_device *chg_dev, int hvdcp3_type)
{
	struct bq2597x *bq = charger_get_data(chg_dev);
...
static int bq2597x_set_bq_chg_done(struct charger_device *chg_dev, int enable)
{
	struct bq2597x *bq = charger_get_data(chg_dev);
...
static int bq2597x_get_bq_chg_done(struct charger_device *chg_dev, int *bq_chg_done)
{
	struct bq2597x *bq = charger_get_data(chg_dev);
...
static int bq2597x_set_hv_charge_enable(struct charger_device *chg_dev, int enable)
{
	struct bq2597x *bq = charger_get_data(chg_dev);
...
static int bq2597x_get_hv_charge_enable(struct charger_device *chg_dev, int *hv_chg_enable)
{
	struct bq2597x *bq = charger_get_data(chg_dev);
...
static int bq2597x_get_vbat_volt(struct charger_device *chg_dev, u32 *vbat_volt)
{
	struct bq2597x *bq = charger_get_data(chg_dev);
	int ret, result;
	...
	*vbat_volt = bq->vbat_volt;

	return ret;
}

static int bq2597x_get_ibat_curr(struct charger_device *chg_dev, u32 *ibat_curr)
{
	struct bq2597x *bq = charger_get_data(chg_dev);
	int ret, result;
	...
	*ibat_curr = bq->ibat_curr;

	return ret;
}

static int bq2597x_get_battery_temp(struct charger_device *chg_dev, int *bat_temp)
{
	struct bq2597x *bq = charger_get_data(chg_dev);
	int ret, result;
	...
	*bat_temp = bq->bat_temp;

	return ret;
}

static int bq2597x_get_vbus_volt(struct charger_device *chg_dev, u32 *vbus_volt)
{
	struct bq2597x *bq = charger_get_data(chg_dev);
	int ret, result;
	...
	*vbus_volt = bq->vbus_volt;

	return ret;
}

static int bq2597x_get_ibus_curr(struct charger_device *chg_dev, u32 *ibus_curr)
{
	struct bq2597x *bq = charger_get_data(chg_dev);
	int ret, result;
	...
	*ibus_curr = bq->ibus_curr;

	return ret;
}

static int bq2597x_get_bus_temp(struct charger_device *chg_dev, int *bus_temp)
{
	struct bq2597x *bq = charger_get_data(chg_dev);
	int ret, result;
	...
	*bus_temp = bq->bus_temp;

	return ret;
}

static int bq2597x_get_die_temp(struct charger_device *chg_dev, int *die_temp)
{
	struct bq2597x *bq = charger_get_data(chg_dev);
	int ret, result;
...
{
	int work_mode = BQ25970_ROLE_STDALONE;

	bq2597x_enable_wdt(bq, false);

	bq2597x_set_charge_mode(bq, SC8551_CHARGE_MODE_DIV2);
	...
	bq2597x_get_work_mode(bq, &work_mode);
	if (bq->cfg->sc8551a_dual_standalone
			&& work_mode == BQ25970_ROLE_STDALONE
			&& bq->mode == BQ25970_ROLE_SLAVE)
		bq2597x_disable_vbus_inrge_det(bq, true);

	return 0;
}

static int bq2597x_set_present(struct charger_device *chg_dev, bool present)
{
	struct bq2597x *bq = charger_get_data(chg_dev);
...
static int bq2597x_get_present(struct charger_device *chg_dev, bool *present)
{
	struct bq2597x *bq = charger_get_data(chg_dev);
...
unsigned char g_reg_value_bq2597x;
static ssize_t show_bq2597x_access(struct device *dev,
				   struct device_attribute *attr, char *buf)
{
	pr_info("[%s] 0x%x\n", __func__, g_reg_value_bq2597x);
	return sprintf(buf, "0x%x\n", g_reg_value_bq2597x);
}

static ssize_t store_bq2597x_access(struct device *dev,
				    struct device_attribute *attr,
				    const char *buf, size_t size)
{
	struct bq2597x *bq = dev_get_drvdata(dev);
	int ret = 0;
	char *pvalue = NULL, *addr, *mask, *val;
	...
	if (buf != NULL && size != 0) {
		...
		if (size > 3) {
			...
			if (size > 6) {
				mask = strsep(&pvalue, " ");
				ret = kstrtou32(mask, 16, (unsigned int *)&reg_mask);
			}
			val = strsep(&pvalue, " ");
			ret = kstrtou32(val, 16, (unsigned int *)&reg_value);

			reg_address2 = reg_address;
			reg_mask2 = reg_mask;
			reg_value2 = reg_value;

			if (size > 6) {
				pr_info("[%s] write bq2597x reg 0x%x with mask 0x%x, value 0x%x !\n",
						__func__, reg_address2, reg_mask2, reg_value2);
				ret = bq2597x_update_bits(bq, reg_address2, reg_mask2, reg_value2);
			} else {
				pr_info("[%s] write bq2597x reg 0x%x, value 0x%x !\n",
						__func__, reg_address2, reg_value2);
				ret = bq2597x_write_byte(bq, reg_address2, (u8)reg_value2);
			}
		} else {
			ret = kstrtou32(pvalue, 16, (unsigned int *)&reg_address);
			ret = bq2597x_read_byte(bq, (u8)reg_address, &g_reg_value_bq2597x);
			pr_info("[%s] read bq2597x reg 0x%x with value 0x%x !\n",
					__func__, reg_address, g_reg_value_bq2597x);
		}
	}
	return size;
}

static DEVICE_ATTR(access, 0664, show_bq2597x_access,
		   store_bq2597x_access);
				...
				union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
		bq2597x_check_charge_enabled(bq->chg_dev, &bq->charge_enabled);
	...
	case POWER_SUPPLY_PROP_MODEL_NAME:
		...
		else if (bq->mode == BQ25970_ROLE_STDALONE)
			val->strval = "bq2597x-standalone";
		else
			val->strval = "bq2597x-unknown";
		break;
	case POWER_SUPPLY_PROP_TI_CHARGE_MODE:
		sc8551_get_charge_mode(bq->chg_dev, &val->intval);
		break;
	default:
		return -EINVAL;
				       ...
				       const union power_supply_propval *val)
{
	...
	switch (prop) {
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
		bq2597x_enable_charge(bq->chg_dev, val->intval);
		bq2597x_check_charge_enabled(bq->chg_dev, &bq->charge_enabled);
		bq_info("POWER_SUPPLY_PROP_CHARGING_ENABLED: %s, en_bit:%d\n",
				val->intval ? "enable" : "disable", bq->charge_enabled);
		break;
	case POWER_SUPPLY_PROP_PRESENT:
		bq2597x_set_present(bq->chg_dev, !!val->intval);
		break;
	case POWER_SUPPLY_PROP_TI_SET_BUS_PROTECTION_FOR_QC3:
		bq2597x_set_bus_protection(bq->chg_dev, val->intval);
		break;
	case POWER_SUPPLY_PROP_BQ_CHARGE_DONE:
		bq->bq_charge_done = val->intval;
		break;
	case POWER_SUPPLY_PROP_HV_CHARGE_ENABLED:
		bq->hv_charge_enable = val->intval;
		break;
	case POWER_SUPPLY_PROP_TI_CHARGE_MODE:
		sc8551_set_charge_mode(bq->chg_dev, val->intval);
		break;
	default:
		return -EINVAL;
	}
...
static const struct charger_ops bq2597x_chg_ops = {
	.enable = bq2597x_enable_charge,
	.is_enabled = bq2597x_check_charge_enabled,

	.set_bus_protection = bq2597x_set_bus_protection,
	.set_present = bq2597x_set_present,
	...
	.set_bypass_enable = sc8551_set_bypass_mode_en,
	.get_bypass_enable = sc8551_get_bypass_mode_en,
	.set_chg_mode = sc8551_set_charge_mode,
	.get_chg_mode = sc8551_get_charge_mode,

	.get_battery_present = bq2597x_get_battery_present,
	.get_vbus_present = bq2597x_get_vbus_present,

	.get_vbat_adc = bq2597x_get_vbat_volt,
	.get_ibat_adc = bq2597x_get_ibat_curr,
	.get_vbus_adc = bq2597x_get_vbus_volt,
	.get_ibus_adc = bq2597x_get_ibus_curr,
	.get_battery_temp = bq2597x_get_battery_temp,
	.get_bus_temp = bq2597x_get_bus_temp,
	.get_die_temp = bq2597x_get_die_temp,

	.get_alarm_status = bq2597x_get_alarm_status,
	.get_fault_status = bq2597x_get_fault_status,
					...
					const struct i2c_device_id *id)
{
	...
	if (bq->cfg->sc8551a_dual_standalone) {
		if (!strcmp(bq->client->name, "bq2597x-slave"))
			bq->mode = BQ25970_ROLE_SLAVE;
		else if (!strcmp(bq->client->name, "bq2597x-master"))
			bq->mode = BQ25970_ROLE_MASTER;
	}
	...
	bq->chg_props.alias_name = bq->psy_desc.name;
	bq->chg_dev = charger_device_register(chg_name, bq->dev,
			bq, &bq2597x_chg_ops, &bq->chg_props);
	if (IS_ERR_OR_NULL(bq->chg_dev)) {
		bq_err("charger device register fail\n");
		goto err_1;
	}
