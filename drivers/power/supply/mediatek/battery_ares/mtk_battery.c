...
static enum power_supply_property battery_props[] = {
	...
	POWER_SUPPLY_PROP_CHARGE_CONTROL_SCENE,
		...
		union power_supply_propval *val)
{
	...
	pr_info("nchg:capacity:%d, data->night_chg_flag:%d, pre_night_chg_flag:%d, input_suspend:%d\n, fcc:%d",
			data->BAT_CAPACITY, data->night_chg_flag, pre_night_chg_flag, input_suspend, fcc);

	if (pre_night_chg_flag != data->night_chg_flag) {
		if (data->night_chg_flag && data->BAT_CAPACITY >= 80) {
			pval.intval = data->night_chg_flag;
			charger_manager_set_input_suspend(pval.intval);
	...
	union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_INPUT_SUSPEND:
		...
		break;
	case POWER_SUPPLY_PROP_CHARGE_CONTROL_SCENE:
		val->intval = charger_manager_get_prop_system_temp_scene();
		break;
	...
	case CHARGER_NOTIFY_START_CHARGING:
		{
			...
			if (chr_type == CHARGER_UNKNOWN) {
				bm_err("%s: chg type unknown return\n", __func__);
				return NOTIFY_DONE;
			}
