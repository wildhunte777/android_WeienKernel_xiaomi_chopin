...
#define WL2866D_CLASS_NAME       "camera_ldo"
...
#if defined(CONFIG_DRM_PANEL_K10_42_02_0A_DSC_CMD)
	const char *str = "wl2866d-dvdd2";
	/*remove wl2866d1-dvdd2 regulator disabled first time*/
	if (0 == wl2866d0_dvdd2) {
		if (0 == fan_reg->index && strcmp(fan_reg->rdesc.name,str) == 0) {
			wl2866d_debug("init phase: [%s][%d] regulator disabled\n", fan_reg->rdesc.name, fan_reg->index);
			wl2866d0_dvdd2 = 1;
			return 0;
		}
	}
...
#if defined(CONFIG_DRM_PANEL_K10A_36_02_0A_DSC_VDO) || defined(CONFIG_DRM_PANEL_K10A_42_02_0B_DSC_VDO)
	const char *str = "wl2866d-dvdd2";
	/*remove wl2866d2-dvdd2 regulator disabled first time*/
	if (0 == wl2866d1_dvdd2) {
		if (1 == fan_reg->index && strcmp(fan_reg->rdesc.name,str) == 0) {
			pr_info("init phase: [%s][%d] regulator skip disabled\n", fan_reg->rdesc.name, fan_reg->index);
			wl2866d1_dvdd2 = 1;
			return 0;
		}
	}
